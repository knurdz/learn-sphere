...
Provider secrets (Foundry, LiveKit) belong only in `api/.env.local`, never in the Flutter app. The worker keeps its own `agent/.env.local` with the LiveKit and Beyond Presence keys.
